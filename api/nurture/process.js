  ...
  if (row.nurture_step === 0) {
    return {
      ...
      html: `
        ...
        <p>Your current risk score is <strong>${escapeHtml(score)}</strong>${location ? ` for ${escapeHtml(location)}` : ""}.</p>
  ...
  if (row.nurture_step === 1) {
    return {
      ...
      html: `
        ...
        <p>Your current score is <strong>${escapeHtml(score)}</strong>.</p>
...
function getNextSchedule(stepJustSent) {
  ...
  if (stepJustSent === 0) {
    return {
      ...
      nurture_step: 1,
      nurture_next_send_at: new Date(now.getTime() + 24 * 60 * 60 * 1000).toISOString(),
    };
  }
  ...
  if (stepJustSent === 1) {
    return {
      nurture_status: "active",
      nurture_step: 2,
      nurture_next_send_at: new Date(now.getTime() + 2 * 24 * 60 * 60 * 1000).toISOString(),
    };
  }
    ...
    for (const row of rows || []) {
      ...
      if (row.callback_requested === true) {
        results.push({
          ...
          ok: false,
          skipped: true,
          reason: "Callback requested; nurture should not run",
        });
        continue;
      }
      ...
      try {
        ...
        await supabase
          ...
          .update({
            email_status: "failed",
            email_error: sendError.message || "Nurture send failed",
        ...
        results.push({
          ...
          email: row.email,
          ok: false,
          error: sendError.message || "Nurture send failed",
        });
      }
    }
